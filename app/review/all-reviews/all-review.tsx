...
export const AllReviews = () => {
    ...
    return (
        <>
            ...
            <div className="md:mt-16 mt-28 px-6 lg:px-10">
                <div className="flex items-center justify-between mb-8">
                    <Link 
                        href="/#testimonials" 
                        className="inline-flex items-center gap-2 text-muted-foreground hover:text-[#FE705A] transition-colors duration-200 group"
                    >
                        <CircleArrowLeft className="w-5 h-5 group-hover:translate-x-[-2px] transition-transform duration-200" />
                        <span className="text-sm font-medium">Back to Home</span>
                    </Link>
                    <div className="text-center">
                        <h1 className="text-2xl lg:text-3xl font-bold text-foreground">
                            All Client <span className="text-[#FE705A]">Reviews</span>
                        </h1>
                        <p className="text-sm text-muted-foreground mt-1">
                            {items?.length || 0} verified testimonials
                        </p>
                    </div>
            ...
            {error && !loading && (
                <div className="flex justify-center items-center min-h-[50vh]">
                    <Error msg="Failed to load reviews. Please check your connection and try again." />
                </div>
            )}

            {/* Reviews Grid */}
            {!loading && !error && items && (
                <div className="flex flex-wrap gap-6 justify-center items-stretch w-[95%] mx-auto pb-16">
                    {items.map((item, idx) => (
                        <div
                            className="w-full max-w-md flex-shrink-0"
                            key={idx}
                        >
                            <Card className="bg-tertiary dark:bg-secondary/40 p-6 h-full flex flex-col hover:shadow-lg transition-all duration-300 hover:scale-[1.02] border border-border/30 hover:border-[#FE705A]/20">
                                        ...
                                        <div className="flex flex-col flex-1 min-w-0">
                                            <div className="flex justify-between w-full items-start gap-3">
                                                <div className="space-y-1 flex-1 min-w-0">
                                                    <CardTitle className="text-base font-semibold dark:!text-purple truncate">
                                                        {item.userFullname}
                                                    </CardTitle>
                                ...
                                {/* Rating */}
                                <div className="flex gap-1 mb-3 items-center flex-shrink-0">
                                    {[...Array(5)].map((_, i) => (
                                        <Star
                                ...
                                <div className="flex-1 flex flex-col">
                                    <div className="text-sm text-muted-foreground leading-relaxed text-justify hyphens-auto tracking-wide flex-1">
                                        {item.reviewText.length > 400 ? (
                                            <span className="space-y-3">
                                                ...
                                                <button 
                                                    ...
                                                    className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-[#FE705A] hover:bg-[#FE705A]/10 rounded-lg transition-all duration-300 group border border-[#FE705A]/20 hover:border-[#FE705A]/40 mt-auto"
                                                >
                                ...
                                {/* Date */}
                                <div className="mt-4 pt-3 border-t border-border/30 flex-shrink-0">
                                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                        <Calendar className="w-3 h-3" />
                                        <span>Reviewed on {formatDate(item.createdAt)}</span>
                                    </div>
                                </div>
                            </Card>
                        </div>
            ...
            {/* Professional Modal - Same as main component */}
